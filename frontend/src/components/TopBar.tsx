...
export default function TopBar() {
  ...
  const { rootName, rootPath, pickRoot, setRootPath } = useLocalFilesystemStore();
          ...
          {settingsOpen && (
            <div className="absolute top-full right-0 mt-2 w-80 rounded-lg bg-anvil-panelHover border border-anvil-border shadow-xl z-50 overflow-hidden p-4 space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold text-anvil-muted uppercase tracking-wider">Local Project Folder</span>
              ...
              <div className="space-y-2">
                <button
                  onClick={() => pickRoot()}
                  className="w-full flex items-center justify-center gap-2 px-3 py-2 rounded bg-anvil-accent hover:bg-blue-600 text-white text-xs font-medium transition"
                >
                  <FolderOpen className="w-3.5 h-3.5" />
                  {rootName ? 'Change folder' : 'Choose project folder'}
                </button>
                {rootName ? (
                  <p className="text-[10px] text-anvil-success break-all">{rootName}</p>
                ) : (
                  <p className="text-[10px] text-anvil-muted">
                    Required for the agent to read/write files on your machine.
                  </p>
                )}

                <div className="space-y-1">
                  <label className="block text-[10px] text-anvil-muted uppercase">VS Code: folder path (absolute)</label>
                  <input
                    type="text"
                    value={rootPath || ''}
                    onChange={(e) => setRootPath(e.target.value)}
                    placeholder="C:\\Projects\\MyProject"
                    className="w-full px-2 py-1.5 rounded bg-anvil-bg border border-anvil-border text-xs text-anvil-text outline-none focus:border-anvil-accent"
                  />
                  <p className="text-[10px] text-anvil-muted">
                    Used to root the embedded VS Code: tab after a write.
                  </p>
                </div>
              </div>

              <div className="flex items-center justify-between pt-2 border-t border-anvil-border">
