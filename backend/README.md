...
agent involved — for the MCP-backed filesystem adapter that means connecting
exactly the way ADK's `MCPToolset` will and confirming every tool in scope is
present; for the custom cad adapter it's a direct function call since there's
no server in the loop. Filesystem exercises write → list → read → get_file_info
plus a boundary check that paths outside the sandbox are rejected; cad builds a
