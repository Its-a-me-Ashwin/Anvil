...
def _build_instruction(ctx) -> str:
    ...
    return (
        ...
        "changes — it overwrites rather than averages. When the user first describes "
        "a project and the parts and tools they already own, make an initial 1-5 "
        "estimate for each of the five categories the project touches and set it with "
        "set_skill_level right then — this seeds the skill radar so you can pitch "
        "things at the right level from the very next answer. None of this needs to "
        "show up in your reply text (see TESTING MODE below) — the tool call itself "
        "is what matters, not narrating it.\n"
